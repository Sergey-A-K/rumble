...
/*
 ...
 =======================================================================================================================
 */
void mailman_copy_letter( mailman_bag * bag, mailman_folder * sourceFolder, mailman_folder * destFolder,

    uint64_t start, uint64_t stop, uint32_t UID) {

    printf("mailman_copy_letter()\n");
    if (!bag || !sourceFolder || !destFolder) return;

    if (stop == 0) stop = start;
    rumble_rw_start_write(bag->lock);
    for (uint32_t i = 0; i < sourceFolder->size; i++) {
        mailman_letter * letter = &sourceFolder->letters[i];
        if (letter->inuse) {
            if ((UID && (letter->id >= start && letter->id <= stop)) || (!UID && (i + 1 >= start && 1 < i))) { // TODO
...
//     rumble_parsed_letter    *letter;
    ...
    while (!feof(fp)) {
        if (fgets(line, 1024, fp)) {
            ...
            } else // Done with headers, browse through them and look for a boundary if any
            if (headers == 0) {
