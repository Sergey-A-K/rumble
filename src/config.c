    ...
    if (config) {
        ...
        while (!feof(config)) {
            ...
            if (!ferror(config)) {
                ...
                if (sscanf(line, "<%511[^ \t>]%*[ \t]%511[^\n]>", key, value) >= 1) {
                    ...
                    if (!strcmp(key, "if")) {
                        ...
                        if (sscanf(value, "defined(%128[^) \t])", ckey) == 1)
                            if (rumble_has_dictionary_value(master->_core.conf, ckey)) ignore &= 0xFFFFFFFE;
                ...
                if (sscanf(line, "%511[^# \t]%*[ \t]%511[^\n]", key, value) == 2 && !ignore) {
                    rumble_string_lower(key);
                    rumble_add_dictionary_value(master->_core.conf, key, value);
            ...
            if (!ferror(config)) {
                ...
                if (sscanf(line, "<%511[^ \t>]%*[ \t]%511[^\n]>", key, value) >= 1) {
                    ...
                    if (!strcmp(key, "if")) {
                        ...
                        if (sscanf(value, "defined(%128[^) \t])", ckey) == 1)
                            if (rumble_has_dictionary_value(configFile, ckey)) ignore &= 0xFFFFFFFE;
                        if (sscanf(value, "exists(%128[^) \t])", ckey) == 1)
                            if (rumble_file_exists(ckey)) ignore &= 0xFFFFFFFE;
                    }
