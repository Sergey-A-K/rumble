...
int smtp_deliver_feed(rumble_mailbox *user, mqueue *item, masterHandle *master) {
    ...
    const char mm[] = "%s/smtp_deliver-XXXXXX";
    tempfile = (char *) calloc(1, strlen(path) + strlen(mm));
    sprintf(tempfile, mm, path);
...
int smtp_deliver_foreign(mqueue *item, masterHandle *master, const char *host) {
    ...
    maxAttempts = atoi(rumble_get_dictionary_value(master->_core.conf, "deliveryattempts"));
    maxAttempts = maxAttempts ? maxAttempts : 5;

    retryInterval = atoi(rumble_get_dictionary_value(master->_core.conf, "retryinterval"));
    retryInterval = retryInterval ? retryInterval : 360;

    ignmx = rumble_get_dictionary_value(master->_core.conf, "ignoremx");
    badmx = dvector_init();
...
Mailer Daemon at %s on Rumble Mail Server v/%u.%u.%u\r\n",
            sender, recipient, reason, rumble_get_dictionary_value(master->_core.conf, "servername"), RUMBLE_MAJOR, RUMBLE_MINOR, RUMBLE_REV);
...
rumble_sendmail_response *rumble_send_email(
    ...
    while (c.socket) {
        ...
        /* Do a MAIL FROM */
        if (rumble_has_dictionary_value(res->flags, "SIZE")) {
