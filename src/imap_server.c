...
ssize_t rumble_server_imap_login(masterHandle *master, sessionHandle *session, const char *parameters, const char *extra_data) {
    ...
    if ( sscanf(parameters, "\"%256[^\" ]\" \"%256[^\" ]\"", user, pass) == 2 or
         sscanf(parameters, "\"%256[^\" ]\" %255s", user, pass) == 2 or
         sscanf(parameters, "%255s \"%256[^\" ]\"", user, pass) == 2 or
            ...
            if (line) {
                ...
                if (addr) {
                    ...
                    if (imap->account) {
                        ...
                        imap->bag = mailman_get_bag(imap->account->uid,
                                                    strlen(imap->account->domain->path) ? imap->account->domain->path : rumble_get_dictionary_value(
...
ssize_t rumble_server_imap_select(masterHandle *master, sessionHandle *session, const char *parameters, const char *extra_data) {
    ...
    if (imap->account) {
        ...
        if (folder) {
            ...
            if (first) {
                rumble_comm_printf(session, "* OK [UNSEEN %"PRIu64 "] Message %"PRIu64 " is the first unseen message.\r\n", first, first);
            }
...
ssize_t rumble_server_imap_create(masterHandle *master, sessionHandle *session, const char *parameters, const char *extra_data) {
    ...
    if (args && args->argc == 1) {
        ...
        if (newFolder) rumble_comm_printf(session, "%s NO CREATE failed: Duplicate folder name.\r\n", extra_data);
        else {
