...
void *rumble_smtp_init(void *T) { // TODO BUG?
    ...
    //printf("rumble_smtp_init, line %d\n", __LINE__);
    ...
    while (1) {
        ...
        while (rc != RUMBLE_RETURN_FAILURE) {
...
#if (RUMBLE_DEBUG & RUMBLE_DEBUG_COMM)
                rumble_debug(NULL, "smtp", "%s said: %s %s", session.client->addr, cmd, arg); // TODO
#endif
            ...
            if (rc == RUMBLE_RETURN_IGNORE) {
                ...
                rumble_debug(NULL, "smtp", "a module replied to %s instead of me", session.client->addr); // TODO
        ...
        if (rc == 421) {
            ...
            rumble_comm_send(sessptr, rumble_smtp_reply_code(421422));   /* timeout! */
...
ssize_t rumble_server_smtp_helo(masterHandle *master, sessionHandle *session, const char *parameters, const char *extra_data) {
    ...
    if (rc != RUMBLE_RETURN_OKAY) return (rc);
    strictHelo = atoi(rumble_get_dictionary_value(master->_core.conf, "enforcefqdn"));
...
ssize_t rumble_server_smtp_data(masterHandle *master, sessionHandle *session, const char *parameters, const char *extra_data) {
    ...
    if (!log) merror();
    now = rumble_mtime(); // TODO Localtime ?
